...
- **实时流式转写**：会议中直接开麦直播，停止后落成和上传模式一致的原文与录音，进入同样的校对 → 整理 → 总结流程；支持标准模式（内置引擎，全平台）与增强模式（GPU 引擎，WSL/Linux + NVIDIA 独显）
- **历史会议管理**：给会议打标签 + 按标签筛选、对已处理完的会议改名、删除可选「移到回收站」（文件保留可找回）或「完全删除」
...
**v0.4 新增（实时流式转写）**：
- **双引擎实时转录**：会议中开麦直播，停止后自动落成 `audio_live.wav` + `raw.json`，直接进入「待整理」状态，后续校对 / 整理 / 总结流程与上传模式完全一致
- **标准模式**：内置 FunASR 流式引擎（`paraformer-zh-streaming`），macOS / WSL / Linux 通用，无需额外配置
- **增强模式**：转发到 Fun-ASR-Nano vLLM GPU sidecar（`ws://localhost:10095`），方言 / 口音 / 远场识别更准；仅 WSL/Linux + NVIDIA 独显可用，Mac 前端自动灰色禁用
- **统一入口**：首页「实时记录」标签，实时页可切换引擎，选择增强模式时自动探测 sidecar 是否就绪
...
| **增强模式** | GPU 实时引擎 | 仅 WSL/Linux + NVIDIA 独显 | NVIDIA 独显 8GB+ 显存（推荐 12GB+）|

- **标准模式**默认开启，所有平台可用，无需额外配置。
- **增强模式**面向方言 / 口音 / 远场等重场景，准确率更高；Mac 不支持，前端选择器会灰色禁用并提示原因。
- 增强模式需要单独启动一个 GPU sidecar 服务（`ws://localhost:10095`），可在同一台 WSL/Linux 机器上部署。配置 `config.yaml` 的 `asr.stream_engine: sidecar` 与 `asr.sidecar_url` 即可切换。
