...
**v0.6 已完成（离线核心）**：批量上传 + 队列、说话人手动合并 / 逐句拆分（纯元数据重映射）、整理版与纪要的 docx 导出、导出选项（是否带说话人 / 时间戳）、校对增强（倍速 + 跳静音）、pipeline 解锁（多文件转换 / 整理并行）。

**暂未做**：议程章节切分（时间轴稳定后做）。增强模式（GPU sidecar，方言 / 口音 / 远场更准）作为**远期目标**，当前前后端一致禁用。
