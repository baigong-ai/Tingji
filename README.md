...
- **实时流式转写**：会议中直接开麦直播，停止或断线后原文与录音**立即**落盘，后台自动二次识别升级为带准确时间戳与说话人分离的精稿，随后进入与上传模式一致的校对 → 整理 → 总结流程；标准模式（内置引擎，全平台开箱即用）；增强模式（GPU 引擎）暂不提供，实时页已只保留标准模式
...
服务重启时正在处理的任务会卡在中间状态。除了详情页手动点「恢复任务」，还可以装一个每小时巡检的 cron 自动续跑（实时录音中断的会议，录音已落盘的会自动恢复识别，未能保存的标记为失败）：
...
**v0.7.1 新增（实时保存可靠性）**：
- **会议结束立刻见稿**：实时记录停止或断线后，录音与流式原文**立即**落盘展示；系统在后台继续跑离线二次识别（期间状态显示「识别中」），完成后页面自动刷新为带准确时间戳与说话人分离的精稿，校对 / 整理 / 总结流程不变
- **修复「会议结束后一直显示实时中、页面无内容」**：此前要先等离线二次识别（含模型冷加载数分钟，GPU 停滞时可达十分钟以上）全部跑完才更新会议状态，期间看起来像记录丢失，识别卡死则状态永久卡住；现在状态与耗时识别解耦，秒级落定
- **失败有兜底**：二次识别失败或超时保留流式稿，详情页点「恢复任务」可重跑；服务重启中断的实时会议，录音已落盘的自动从磁盘恢复识别，不再一律报「录音丢失」
...
除「上传录音再处理」外，听记支持会议中直接开麦实时转写。停止或断线后，录音与流式原文**立即**落盘展示，无需守在页面前等待；系统在后台继续完成带准确时间戳与说话人分离的二次识别，完成后页面自动刷新，随后进入与上传模式一致的校对 → 整理 → 总结流程。
