...
- **说话人自动区分**：基于 CAM++ 音色聚类，自动分清"谁在说话"，可改成真实名字并同步到所有视图与导出；cam++ 偶尔把多人并一人 / 一人拆两人，可**手动合并说话人**或**逐句改归属（含拆出新说话人）**，纯元数据重映射、不动音频不重跑 ASR
- **发言人时间轴**：顶部一条按累计发言时长的比例色条，一眼看出谁说得多；点击定位该说话人第一句，播放时高亮当前说话人
- **逐句时间戳**：点任意句定位音频（不强制播放），播放时当前句自动高亮 + 滚动跟随
- **纪要时间戳回跳**：决议 / 待办 / 待讨论每条可带 `[m:ss]` 时间戳，点击直接跳回音频现场，“这真是会上定的吗”一键核对；整理稿小节标题同样带发言开始时间，导出纪要时时间戳内联保留
- **个人笔记层**：把鼠标移到任意原句上点 ✎ 写自己的批注，笔记锚定句子、独立存储，**重新整理永不覆盖**；总结页顶部「我的笔记」按时间汇总，点击跳回锚定句；也支持不锚定句子的全局笔记；纪要导出可附「个人笔记」一节
- **议题时间轴**：说话人条下方第二条时间轴，把会议切成「背景 / 提出问题 / 讨论 / 结论 / 行动 / 闲聊」的议题段，点击跳到该议题开头、播放时高亮当前议题；LLM 生成初值，人可校正（改名 / 改阶段 / 调边界 / 删段），手动改过后重新整理不会覆盖
- **原文↔整理对照**：左右双栏按时间戳精确关联，hover 高亮、点击跳转音频、播放同步
- **手动校对 + 热词 + 倍速/跳静音**：双击句子纠正识别错误（可顺手改说话人归属、加入热词）；音频条支持 0.75×–2× 倍速与"跳静音"（自动跳过句间空隙），校对更快
- **批量上传**：一次拖入多个录音文件批量转录，首页队列逐个显示进度，识别按队列依次进行、转换与整理跨文件并行
...
- **导出** `.md` / `.docx`（整理版）、`.md` / `.docx`（纪要，只拿四段总结）、`.txt` / `.srt`（原文，可选是否带说话人 / 时间戳）
- **实时流式转写**：会议中直接开麦直播，停止后落成和上传模式一致的原文与录音，进入同样的校对 → 整理 → 总结流程；标准模式（内置引擎，全平台开箱即用）；增强模式（GPU 引擎）暂不提供，实时页已只保留标准模式
...
**纪要能回现场，笔记不会丢（v0.7）。** 纪要每条决议、待办后面跟一个 `[m:ss]` 时间戳，点一下直接跳回那句话的音频现场；鼠标移到原文任意句上点 ✎ 写批注，笔记存在独立的 `notes.json` 里，重新整理纪要也不会覆盖它；顶部新增议题时间轴，一眼看清这场会怎么走过来的（提出问题 → 讨论 → 结论 → 行动），点任意议题段回到现场，分段不满意可以手动改。
...
核心链路可用：上传 → 识别（带说话人分离 + 时间戳）→ 校对 → 一键整理 + 结构化纪要（条目带时间戳）+ 议题时间轴 + 个人笔记；历史会议支持标签、改名、删除与回收站恢复；实时流式转写（标准模式）。

**v0.7 新增（会后消费层：追溯、笔记、议题）**：
- **纪要时间戳**：整理稿小节标题带 `[m:ss]` 发言时间锚点，纪要决议 / 待办 / 待讨论条目带可点击时间戳 chip，一键回跳音频现场；导出纪要时时间戳内联保留。模型输出不了带时间的 JSON 时自动降级为普通条目，旧会议数据照常渲染
- **个人笔记层**：原句 hover 出 ✎ 入口写锚定批注（也可写不锚定的全局笔记），独立 `notes.json` 存储，**重新整理不覆盖**；总结页顶部「我的笔记」折叠区按时间汇总、点击跳回锚定句；纪要导出可附「个人笔记」一节（`notes=false` 关闭）
- **议题时间轴**：说话人条下方新增议题分段条（背景 / 提出问题 / 讨论 / 结论 / 行动 / 闲聊 六种阶段配色），点击跳转、播放高亮当前议题、hover 预览该段首句；LLM 生成初值，可手动改名 / 改阶段 / 调边界 / 删段，手动修改后重新整理不再自动覆盖；长会按约 25 分钟窗口分段生成再合并边界；也可在详情页单独生成或重新生成
- **增强模式（GPU sidecar）暂不提供**：实时页移除引擎选择器，只保留标准模式；后端仍拒绝 `sidecar` 选中（直连 API 亦然）
...
- **安全**：说话人名注入 XSS 修复；增强模式（GPU sidecar）后端同步禁止选中（后改为暂不提供，见 v0.7）
...
- **增强模式（GPU 引擎）**：原计划转发到 Fun-ASR-Nano vLLM GPU sidecar（方言 / 口音 / 远场识别更准）；因部署门槛高，v0.7 起从前端移除、暂不提供（后端 `app/stream.py` 保留实现）
- **统一入口**：首页「实时记录」标签进入实时页
...
**v0.6 已完成（离线核心）**：批量上传 + 队列、说话人手动合并 / 逐句拆分（纯元数据重映射）、整理版与纪要的 docx 导出、导出选项（是否带说话人 / 时间戳）、校对增强（倍速 + 跳静音）、pipeline 解锁（多文件转换 / 整理并行）、整理质量保障（假整理回声检测 / 思考开关 / 纠偏 prompt）。

**暂未做**：问纪要（围绕会议内容问答）、纪要组件化（表格 / 分栏 / 流程条）、静态 HTML 页面导出等会后消费层的后续能力。增强模式（GPU sidecar，方言 / 口音 / 远场更准）作为**远期目标**，当前前后端一致禁用。
...
- **增强模式**（GPU 引擎，面向方言 / 口音 / 远场等重场景）暂不提供：需要单独部署 Fun-ASR-Nano vLLM GPU sidecar（`ws://localhost:10095`，仅 WSL/Linux + NVIDIA 独显），门槛较高，v0.7 起实时页已移除该选项；后端仍会拒绝 `sidecar` 选中。实现保留在 `app/stream.py`，未来视需求开放。
...
详情页顶部固定两条时间轴（发言人比例条 + 议题分段条）与工具栏（搜索、Tab），只有内容区滚动。四个 Tab：

- **原文**：带说话人配色和时间戳，点击跳转音频；hover 出 ✎ 可给这句写笔记（有笔记的句子带角标）
- **整理版**：去口语化的通顺文字，小节标题带发言开始时间
- **会议总结**：概述 / 决议 / 待办 / 待讨论（结构化四段；条目带可点击时间戳；顶部「我的笔记」折叠区；模型偶尔不吐严格 JSON 时回退成纯文本，不影响使用）
...
app/
  ...
  stream.py   实时流式引擎（标准 funasr 实现；增强 sidecar 保留但暂不提供）
  llm.py      整理 + 总结（分段 + map-reduce + 条目时间戳）+ 议题分段
  storage.py  data/ 目录 CRUD（会议 / 笔记 / 议题 / 模板 / 日志）
