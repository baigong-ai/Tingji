...
核心链路可用：上传 → 识别（带说话人分离 + 时间戳）→ 校对 → 一键整理 + 结构化纪要；历史会议支持标签、改名、删除。
...
**暂未做**：说话人手动合并 / 拆分、纪要二次编辑保存、docx 导出、导出选项（是否带说话人 / 时间戳）、议程章节切分、**实时流式转写（标准模式内置 + 增强模式需 NVIDIA GPU）**。

## 实时转录（规划中）

除「上传录音再处理」外，听记将支持会议中直接开麦实时转写。停止后自动落成和上传模式一致的原文与录音，进入同样的校对 → 整理 → 总结流程。
...
- 增强模式需要单独启动一个 GPU sidecar 服务（`ws://localhost:10095`），整体方案跑通后再提供部署脚本。
